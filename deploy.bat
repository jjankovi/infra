...
set CUSTOMER_BACKEND_PATH="%CD%/env/%ENV%/%ENV%.backend.tfvars"
set VAR_PATH="%CD%/env/%ENV%/%ENV%.tfvars"
...
@REM terraform -chdir=%TERRAFORM_PATH% init -upgrade -migrate-state
terraform -chdir=%TERRAFORM_PATH% apply -var-file=%VAR_PATH% -parallelism=2
@REM terraform -chdir=%TERRAFORM_PATH% destroy -var-file=%VAR_PATH% -parallelism=2
